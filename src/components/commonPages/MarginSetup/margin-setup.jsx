...
const MarginSetup = () => {
  ...
  // Fetch margin config (left panel)
  useEffect(() => {
    (async () => {
      try {
        const res = await (typeof get === "function"
        ...
        console.error("Error fetching margin config:", e);
      }
    })();
  }, [get]);

  // Fetch product pricing list (right panel)
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch("/api/store-margin");
        ...
        const normalizedProducts = productsArray.map((p) => ({
          ...
          image: p.image,
          selling_margin: p.selling_margin,
          diamond_amount: p.diamond_amount,
          diamond_discount: p.diamond_discount,
          diamond_store_price: p.diamond_store_price,
          diamond_selling_price: p.diamond_selling_price,
          labour_amount: p.labour_amount,
          labour_discount: p.labour_discount,
          labour_store_price: p.labour_store_price,
          labour_selling_price: p.labour_selling_price,
          metal_store_price: p.metal_store_price,
          total_store_price: p.total_store_price,
          total_selling_price: p.total_selling_price,
        }));
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
            {products.map((item, index) => (
              <MarginCard
                ...
                {...item}
                isOpen={selectedProductId === item.product_id}
                onSelect={() => setSelectedProductId(item.product_id)}
              />
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-end items-end gap-3 mt-8 px-4">
        ...
        <Button
          variant="outline"
          className="w-full md:w-auto bg-[#F1F1F2] text-foreground"
          onClick={() => handleSave("previous")}
          disabled={loading}
        >
          Previous
...
const MarginCard = ({
  ...
  return (
    <div className="mt-13 cursor-pointer" onClick={onSelect}>
      <Card className="shadow-none">
        <div className="rounded-t-xl relative p-[17px] w-auto h-[196px] bg-[#FCFCFC]">
          <div className="absolute left-[17px]">
